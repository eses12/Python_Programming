...
weekhours = sum(hours[1:6])
print(f"{sum(hours[1:6])}시간")                                              # ✅ 15시간

# 2️⃣ 가장 많이 공부한 시간 출력하기
maxhour = max(hours)
print(f"{maxhour}시간")                                                    # ✅ 6시간


# 3️⃣ 가장 많이 공부한 요일 출력하기
max_day_index = hours.index(maxhour)
max_day = days[max_day_index]
print(f"{max_day}요일")                                                    # ✅ 토요일
...
print(f"max 점수: {max_score}점, min 점수: {min_score}점")
...
total=sum(price*qty for price, qty in zip(*stocks))
print(f"총액 : {total:,}원")